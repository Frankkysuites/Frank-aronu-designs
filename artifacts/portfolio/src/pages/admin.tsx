...
export default function Admin() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, isCoverImage: boolean = false) => {
    ...
    try {
      ...
      setProfile({ ...profile, image_url: compressed });
      if (isCoverImage) {
        if (isEditingProject) {
  ...
  const handleProjectFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setNewFile({ ...newFile, url: fileUrl, type });
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <Dialog open={isChangingPassword} onOpenChange={setIsChangingPassword}>
        <DialogContent className="max-w-md">
            <DialogDescription className="hidden">Change your admin password</DialogDescription>
